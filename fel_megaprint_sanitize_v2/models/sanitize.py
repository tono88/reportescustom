# -*- coding: utf-8 -*-
from odoo import models
import requests, re
from lxml import etree

NSMAP = {"dte": "http://www.sat.gob.gt/dte/fel/0.2.0"}

def _as_text(x):
    if x is None:
        return ""
    if isinstance(x, bytes):
        try:
            return x.decode("utf-8", errors="ignore")
        except Exception:
            return repr(x)
    return str(x)

def _sanitize_dte_xml(dte_xml: str) -> str:
    if not dte_xml:
        return dte_xml
    try:
        parser = etree.XMLParser(remove_blank_text=True)
        root = etree.fromstring(_as_text(dte_xml).encode("utf-8", errors="ignore"), parser=parser)

        # 1) Frase TipoFrase="4"
        for frase in root.xpath(".//dte:Frase[@TipoFrase='4']", namespaces=NSMAP):
            parent = frase.getparent()
            if parent is not None:
                parent.remove(frase)

        # 2) Impuestos en cero por Item
        for imp in root.xpath(".//dte:Item/dte:Impuestos/dte:Impuesto", namespaces=NSMAP):
            monto = imp.find("dte:MontoImpuesto", namespaces=NSMAP)
            if monto is not None:
                try:
                    val = float((_as_text(monto.text) or "0").strip())
                except Exception:
                    val = 0.0
                if val == 0.0:
                    par = imp.getparent()
                    if par is not None:
                        par.remove(imp)

        # 3) Totales en cero
        for t in root.xpath(".//dte:Totales/dte:TotalImpuestos/dte:TotalImpuesto", namespaces=NSMAP):
            total_attr = (t.get("TotalMontoImpuesto") or "0").strip()
            try:
                val = float(total_attr or "0")
            except Exception:
                val = 0.0
            if val == 0.0:
                par = t.getparent()
                if par is not None:
                    par.remove(t)
                # si se quedó vacío, elimina el contenedor
                if par is not None and len(par) == 0:
                    gpar = par.getparent()
                    if gpar is not None:
                        gpar.remove(par)

        # 4) UnidadMedida en mayúsculas
        for um in root.xpath(".//dte:Item/dte:UnidadMedida", namespaces=NSMAP):
            if um.text:
                um.text = _as_text(um.text).upper()

        # 5) Trim espacios en Municipio
        for mun in root.xpath(".//dte:DireccionReceptor/dte:Municipio | .//dte:DireccionEmisor/dte:Municipio", namespaces=NSMAP):
            if mun.text:
                mun.text = _as_text(mun.text).strip()

        return etree.tostring(root, encoding="utf-8", xml_declaration=False).decode("utf-8", errors="ignore")
    except Exception:
        return dte_xml  # si falla, devolver original

_CDATA_XML_DTE_RE = re.compile(r"(<xml_dte><!\[CDATA\[)(.*?)(\]\]></xml_dte>)", re.DOTALL | re.IGNORECASE)

def _sanitize_request_body(body: str) -> str:
    # si el body trae <xml_dte><![CDATA[ ... ]]></xml_dte>, reemplazar interior
    txt = _as_text(body)
    m = _CDATA_XML_DTE_RE.search(txt)
    if m:
        inner = m.group(2)
        cleaned = _sanitize_dte_xml(inner)
        return txt[:m.start(2)] + cleaned + txt[m.end(2):]
    # si no, tal vez es directamente el DTE
    return _sanitize_dte_xml(txt)

class AccountMove(models.Model):
    _inherit = "account.move"

    def certificar_megaprint(self):
        self.ensure_one()
        original = getattr(super(AccountMove, self), "certificar_megaprint", None)
        if not original:
            return super(AccountMove, self).certificar_megaprint()

        orig_post = requests.post
        def wrapped(url, *args, **kwargs):
            # Sanitizar si el payload va en data bytes/str o en args[0]
            if "data" in kwargs and isinstance(kwargs["data"], (str, bytes)):
                kwargs["data"] = _sanitize_request_body(kwargs["data"])
                if isinstance(kwargs["data"], str):
                    kwargs["data"] = kwargs["data"].encode("utf-8")
            elif args and isinstance(args[0], (str, bytes)):
                new0 = _sanitize_request_body(args[0])
                if isinstance(new0, str):
                    new0 = new0.encode("utf-8")
                args = (new0,) + args[1:]
            # también cubrir json con xml embebido
            if "json" in kwargs and isinstance(kwargs["json"], dict):
                for k in ("xml_dte","xml","dte","xmldte","GTDocumento","gtDocumento","documento","xml_dte_str"):
                    if k in kwargs["json"] and isinstance(kwargs["json"][k], (str, bytes)):
                        kwargs["json"][k] = _sanitize_dte_xml(kwargs["json"][k])
            return orig_post(url, *args, **kwargs)

        try:
            requests.post = wrapped
            return original()
        finally:
            requests.post = orig_post
