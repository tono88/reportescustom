...
class PosReportGenerator(models.Model):
    def _apply_state_filter(self, query, state):
        if not state:
            return query
        alias = 'l' if (' from pos_order as l' in query.lower() or '\nfrom pos_order as l' in query.lower()) else ('pos_order' if (' from pos_order ' in query.lower() or '\nfrom pos_order ' in query.lower()) else 'pos_order')
        lower = query.lower()
        idx = lower.find('group by')
        if idx == -1:
        ...
        if ' where ' in head.lower():
            head += f" AND {alias}.state = '{state}' "
        else:
            head += f" WHERE {alias}.state = '{state}' "
        return head + tail
    ...
    order_state = fields.Selection([
        ('draft','Draft'),('paid','Paid'),('done','Done'),('invoiced','Invoiced'),('cancel','Cancelled')
    ], string='Order State', help='Filter orders by state before grouping')
    ...
    def pos_report(self, option):
        ...
        if getattr(report_values, 'order_state', False):
            data.update({'order_state': report_values.order_state})
    ...
    def get_filter(self, option):
        ...
        st = data.get('order_state')
        if st:
            st_map = {'draft':'Draft','paid':'Pagado','done':'No facturado','invoiced':'Invoiced','cancel':'Cancelled'}
            filters['order_state'] = st_map.get(st, st)
        return filters
    ...
    def get_filter_data(self, option):
        ...
        default_filters = {}
        if report.order_state:
            default_filters['order_state'] = report.order_state
        filter_dict = {
    ...
    def _get_report_sub_lines(self, data):
        ...
        if data.get('report_type') == 'report_by_order':
            ...
            query += " having sum(pos_order_line.qty) <> 0"
            query = self._apply_state_filter(query, data.get('order_state'))
