...
class AccountMove(models.Model):
    ...
    def _fel_call_original_cert(self):
        """Llama al método original del módulo fel_megaprint si existe."""
        original = getattr(super(AccountMove, self), 'certificar_megaprint', None)
        if not original:
            raise UserError(_("No se encontró el método 'certificar_megaprint' del módulo FEL Megaprint."))
        return original()
    ...
    def _fel_certify_with_capture(self, preview_mode=False, preview_confirmed=False):
        ...
        with _RequestsCaptureCtx(self.env, preview_mode=preview_mode, preview_confirmed=preview_confirmed) as cap:
            try:
                res = self._fel_call_original_cert()
                ...
                return wiz.action_open()  # detenemos aquí; no se envió nada aún
    ...
    def action_open_attachments(self):
        """Smart button seguro (sin ref externa) para abrir adjuntos."""
        self.ensure_one()
        return {
