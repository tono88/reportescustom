...
class HrPayslipRun(models.Model):
    ...
    payment_state = fields.Selection([
        ...
        ('to_pay', 'To Pay'),
        ('paid', 'Paid'),
    ], string="Payment State", default='none', tracking=True)
    ...
    def _get_slip_net_amount(self, slip):
        # Try to find a NET rule by code; fallback to sum of worked rules
        net_line = slip.line_ids.filtered(lambda l: l.code and l.code.upper() in ('NET', 'NETO'))
        if net_line:
            return net_line[0].total
        # Fallback: total = sum of inputs - deductions
    ...
    def _create_one_payment_for_batch(self, run, journal, method_line, auto_post=True):
        ...
        # Use a dedicated 'Payroll Clearing' partner if desired, or no partner and set is_internal_transfer
        # Here we create a supplier payment without partner is not allowed, so create a partner 'Payroll Clearing' per company
